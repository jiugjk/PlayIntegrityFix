...
# Dump once; later greps reuse this snapshot.
ALLPROPS="$(resetprop)"

# Microsoft
for PROP in $(printf '%s\n' "$ALLPROPS" | grep -oE 'ro.*.build.tags'); do
    resetprop_if_diff "$PROP" release-keys
done

# Other
for PROP in $(printf '%s\n' "$ALLPROPS" | grep -oE 'ro.*.build.type'); do
...
if printf '%s\n' "$ALLPROPS" | grep -qE "ro.aospa.version|net.pixelos.version|ro.afterlife.version" || [ -f /data/system/gms_certified_props.json ]; then
...
if printf '%s\n' "$ALLPROPS" | grep -qE "persist.sys.pihooks|persist.sys.entryhooks|persist.sys.pixelprops" || [ -f /data/system/gms_certified_props.json ]; then
    PROPS="
